...
- `build_tag_id_features`: `00_assign_data_id` の出力から `unit_tag_id` / `building_tag_id` / `statuses` の `/` 区切りコードをユニーク化し、`tag_ids.parquet`（feature_name と tag_id の対応表）と、`unit_tag_<code>` / `building_tag_<code>` / `status_tag_<code>` を横持ちした `train_tag_ids.parquet`・`test_tag_ids.parquet` を `data/interim/01_tag_id_features/` に生成。
