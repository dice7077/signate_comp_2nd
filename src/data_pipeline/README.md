...
- `join_population_projection`: `01_01_drop_sparse_columns` の出力に対して 1km メッシュ人口予測（2025/2035/2045/2055年の `PTN` 値）を `lon` / `lat` から算出したメッシュ ID でマージし、`data/interim/01_02_join_population_projection/{train,test}.parquet` を生成。47 都道府県分の GeoJSON を一括して `data/interim/lookup_population_mesh/mesh1km_population.parquet` にキャッシュする。
- `split_signate_by_type`: `01_02_join_population_projection` の出力を読み込み、`bukken_type`（1202=戸建、1302=マンション）ごとに分割し、`data/interim/01_03_split_by_type/` に 4 ファイルを書き出し。
