...
- `adjust_mansion_money_room`: `01_03_adjust_mansion_unit_area` の出力を読み込み、マンション行について `unit_house_area_adjusted` で算出した平米単価から桁ズレと思われる `money_room` を検出し、10 で割る/掛ける補正を行った `money_room_adjusted` 列を付与して `data/interim/01_04_adjust_mansion_money_room/{train,test}_{kodate,mansion}.parquet` を出力する（補正できない／必要ない行は元の値をコピー）。
